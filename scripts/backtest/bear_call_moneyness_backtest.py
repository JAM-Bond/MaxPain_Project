...
"""
Bear-call moneyness backtest — symmetric counterpart to bull_put_moneyness_backtest.

OTM (call delta 0.30 short), ATM (call delta 0.50 short), ITM (call delta 0.70 short)
across the 163 ORATS-historical tickers, 45-DTE entry, both held-to-expiry and
50% managed exit, slip=0.50.
"""
...
RESULTS_OUT = ROOT / "data/profile/bear_call_moneyness_results.parquet"

ENTRY_DTE = 45
# bear_call uses C.VERTICAL_SHORT_DELTA directly (no `1 - x` flip — it's already a call delta)
MONEYNESS = {"OTM": 0.30, "ATM": 0.50, "ITM": 0.70}
MGD_50_THRESHOLD = 0.5

logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s")
log = logging.getLogger("bc_moneyness")
...
def simulate_one(slice_by_day, available_days, entry_date, expiration, ticker, moneyness):
    ...
    short_delta_call = pos.legs[0].delta
