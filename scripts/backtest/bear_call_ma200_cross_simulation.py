"""Bear-call with 200-DMA cross-exit rule — per-cycle simulation.

Implements the sealed exit rule from
`docs/BEAR_CALL_MA200_CROSS_EXIT_PREREG.md` §3:

  Close on first day D during the hold where:
    - spot(D) > 1.02 × own 200-DMA(D)
    - AND ≥ 5 trading days have elapsed since entry
    - AND spot was ≤ own 200-DMA at entry (entry-below filter)
    - AND mgd50 has not already fired (existing rule wins on time priority)

Output: data/profile/bear_call_ma200_cross_results.parquet
  One row per (ticker, entry_date, moneyness) — same key as the existing
  bear_call_moneyness_results.parquet — with these new fields:
    - entry_below_ma200    : 0/1 was spot ≤ 200-DMA at entry
...
from structures import open_bear_call, close_cost, intrinsic_value_at_expiry  # noqa: E402
...
RESULTS_OUT = ROOT / "data/profile/bear_call_ma200_cross_results.parquet"

ENTRY_DTE = 45
MONEYNESS = {"OTM": 0.30, "ATM": 0.50, "ITM": 0.70}
MGD_50_THRESHOLD = 0.5

# Sealed cross-exit parameters (pre-reg §3) — mirror of bull-put with reversed
# direction: bull-put uses 0.98 × MA (spot DROPS through MA); bear-call uses
# 1.02 × MA (spot RISES through MA).
CROSS_BUFFER = 1.02             # spot must be > 1.02 × MA200 to trigger
CROSS_MIN_HOLD_TDAYS = 5        # ≥ 5 trading days since entry

logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s")
log = logging.getLogger("bc_cross")
...
def simulate_one(slice_by_day: dict, available_days: list, entry_date: date,
    ...
    """One bear-call cycle with cross-exit + mgd50 + held all tracked."""
    ...
    pos = open_bear_call(entry_chain, pd.Timestamp(entry_date), expiration)
    if pos is None:
        return None
    ...
    # Entry MA200 check — bear-call wants spot ≤ MA200 (mirror of bull-put's ≥)
    ...
    if ma_lookup is not None and entry_ts in ma_lookup.index:
        ...
        if not pd.isna(ma_row["ma200"]) and not pd.isna(ma_row["stkPx"]):
            entry_below_ma200 = int(ma_row["stkPx"] <= ma_row["ma200"])
    ...
    for d in forward_days:
        ...
        # Mirror of bull-put: bear-call fires when spot > 1.02 × MA200 (rallying
        # through the structural-trend line), entered-below, ≥5 tdays since entry
        if (cross_exit_date is None
                and entry_below_ma200 == 1
