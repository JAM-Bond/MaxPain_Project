"""Phase 1 exploratory (bear_call mirror of bull_put_ma_cross_during_hold.py)
— bear_call cycles entered BELOW the underlying's MA, did the position
survive when spot crossed ABOVE the MA during the hold?

Compares two cells per MA window (200-DMA, 50-DMA):
  (A) entered below MA, spot stayed below MA through expiration
  (B) entered below MA, spot crossed above MA at some point during hold
...
a Phase 2 pre-reg (re-simulate with cross-exit rule) is worth doing —
though see project_bull_put_ma200_cross_rejected.md: the bull-put symmetric
Phase 2 was rejected because the cross-exit fired at the vol peak.

Output: data/profile/bear_call_ma_cross_during_hold.parquet (per-cell rollup)
"""
...
OUT_PARQUET = ROOT / "data/profile/bear_call_ma_cross_during_hold.parquet"
...
def main() -> int:
    ...
    print(f"Loaded {len(cycles):,} bear_call cycles across "
    ...
    for ticker, sub in otm.groupby("ticker"):
        ...
        for _, row in sub.iterrows():
            ...
            # Mirror: bear_call entry-gate is BELOW the MA
            entry_below_ma50 = not bool(entry_row["above_ma50"])
            entry_below_ma200 = not bool(entry_row["above_ma200"])
            ...
            # Mirror: did spot cross ABOVE each MA during the hold?
            cross_above_ma50 = bool(window["above_ma50"].fillna(False).any())
            cross_above_ma200 = bool(window["above_ma200"].fillna(False).any())

            tagged_rows.append({
                **row.to_dict(),
                "entry_below_ma50": entry_below_ma50,
                "entry_below_ma200": entry_below_ma200,
                "cross_above_ma50_during_hold": cross_above_ma50,
                "cross_above_ma200_during_hold": cross_above_ma200,
            })
    ...
    for ma_label, entry_col, cross_col in [
        ("200-DMA", "entry_below_ma200", "cross_above_ma200_during_hold"),
        ("50-DMA",  "entry_below_ma50",  "cross_above_ma50_during_hold"),
    ]:
        print("=" * 86)
        print(f"  {ma_label}: bear_call cycles entered BELOW the {ma_label}")
        print("=" * 86)
        below = df[df[entry_col] == True]
        n_below = len(below)
        if n_below == 0:
            print("  (no cycles)")
            continue
        n_crossed = int(below[cross_col].sum())
        n_not_crossed = n_below - n_crossed
        print(f"  Cycles entered below {ma_label}: {n_below:,}")
        print(f"    of which crossed above during hold: {n_crossed:,} "
              f"({n_crossed/n_below*100:.1f}%)")
        print(f"    of which stayed below through expiration: {n_not_crossed:,} "
              f"({n_not_crossed/n_below*100:.1f}%)")
        print()

        # Cell A: stayed below (no cross), held to expiry
        cell_a = below[below[cross_col] == False]
        # Cell B: crossed above, held to expiry
        cell_b = below[below[cross_col] == True]

        a_held = _summarize(f"A) stayed below {ma_label} — HELD", cell_a, "held_pnl")
        b_held = _summarize(f"B) crossed above {ma_label} — HELD", cell_b, "held_pnl")
        a_mgd = _summarize(f"A) stayed below {ma_label} — MGD50", cell_a, "mgd50_pnl")
        b_mgd = _summarize(f"B) crossed above {ma_label} — MGD50", cell_b, "mgd50_pnl")
