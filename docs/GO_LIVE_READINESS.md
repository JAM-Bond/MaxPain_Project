...
| A4 | **Daily order reconciler** (read-only Schwab orders → our book) | T1 | 🟡 | **BUILT 2026-06-10** (`lib/order_reconciler.py` + `scripts/maintenance/reconcile_orders.py`, migration 008 adds open/close_order_id + fees_total). Reads FILLED orders → OPENING inserts status='open', CLOSING matches+closes with **net-of-fees** P/L (fees from `schwab_fills`); idempotent on order_id; ambiguous/zebra/IF **flagged for manual** (never guessed). Dry-run verified on the real HCA order (bull_put 370/365, entry +2.37, fees 1.37); 9/9 unit tests. **Remaining:** schedule the ~16:25 cron at go-live (not wired during paper); live-test the close path when a real close occurs. |
