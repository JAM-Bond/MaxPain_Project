...
| B2 | Failure trapping + dead-man alert (healthchecks.io) covers every job | T1 | 🟡 | Verified: every daily job runs via `run_cron.sh` (per-run failure email) and is in the `cron_heartbeat.py` no-show manifest, with a healthchecks.io dead-man backstop. **Gaps:** `quarterly_refresh` has no no-show check (acceptable — quarterly, still per-run alerted); `reconcile_orders` must be added to the heartbeat manifest at go-live. |
