...
_Updated 2026-06-12 (full audit sweep)._

- **T0:** ✅ (A1b — no execution code exists).
- **T1 ✅ (13):** A1, A1b, A3, A5, B1, B3, C3, C4, D1, D4, F1, F5, H3.
- **DECISIONS MADE 2026-06-12 (user walked the full list) — implementation queued:**
  - **A2 + B2: ACCEPTED 🟡** — weekly manual re-auth + 2-day warning is the standing
    process; documented heartbeat gaps accepted.
  - **C1: FIX CODE on all 4 divergences** — (a) soft-downsize trigger → SPY within
    ±2% of 200-DMA AND trending down (5-day return < 0); (b) bull_put DOWNSIZE
    whenever SPY < 200dma (Stage ≥2 → 0.5); (c) SKIP new ZEBRA entries at regime
    stage ≥1 (anti_zebra exempt — it's the bear-side structure); (d) overlay card
    rendered for EVERY zebra GO/DOWNSIZE (auto-cohort labeled validated, others
    "plan-mandated, per-name validation pending"). Plan v2.5 drafting still deferred.
  - **C5: GATE BOTH** — hard-pause check added to bull_put_earnings track; anti_zebra
    added to BUDGET_CAPS at $300 spot.
  - **F3: CODE THE CHINA SLOT** — correlated-name groups (PDD+BABA) consume one risk
    slot: ≥2 actionable same-OpEx → all DOWNSIZE 0.5 with "shared slot — size both
    half or pick one" reason. Contract counts + book-level budget stay MANUAL through
    go-live (book budget folds into Sept F2 review).
  - **C3 leftover: RE-WIRE** the moneyness/MA-bucket/IF-wing annotation helpers as
    one compact line per card.
