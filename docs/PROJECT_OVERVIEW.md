...
The universe begins at one hundred fifty actively-traded equities and exchange-traded funds with sufficient options-chain depth to admit credit spreads with realistic execution. From this universe, separate cohorts are constructed for each trading structure: a bull-put cohort, a bear-call cohort, an inverted-fly cohort, a ZEBRA (zero-extrinsic back-ratio) cohort split across two tier classifications, and an anti-ZEBRA cohort (the bearish synthetic-short mirror, gated to weak high-volatility regimes). Each cohort is assembled through the same structured selection pipeline.
...
The profile feeds the live system at three integration points. The first is the daily construction-block alert: when more than one trade candidate clears the qualifier on a given evening, the alert checks whether any of the candidates share a non-neutral sensitivity tier on any of the seven dimensions. When two or more candidates share a tier — say, three candidates all in the strongly-positive rate-sensitivity tier — the alert appends a macro-concentration warning identifying the cluster. This is the cross-sector analog of the same-sector concentration cap and surfaces correlation that the sector cap cannot see. The warning is informational; it does not block placement, but it makes the cross-sector concentration visible at the moment trade decisions are made. A tighter, sizing-active version of the same idea handles names that are effectively a single bet despite being distinct tickers — for example the China ADRs PDD and BABA, which move on the same policy and currency shocks: when two or more members of such a correlated-slot group clear on the same expiration, the system halves each of them so the combined position consumes roughly one full risk slot rather than two ("size both half, or pick one").
...
- **Anti-ZEBRA** — The bearish mirror of the ZEBRA: a synthetic-short-equivalent built from puts (2× long in-the-money put + 1× short at-the-money put) that carries roughly −1.00 delta with the same negligible time decay and a maximum loss bounded at the debit. Defined-risk by construction. It only deploys when the bear-call regime gate (H1) is active, and — like the ZEBRA and inverted fly — it is held to a $300 spot-price cap because, unlike the ZEBRA, it carries no long-put overlay to bound the tail.
...
- **Stage 2 — Below trend, unconfirmed.** SPY breaks below its 200-day moving average but IV rank is still under 0.5. New bull puts cut to half size (the below-trend regime is more permissive than the full hard pause but still warrants reduced sizing); new ZEBRAs remain paused; bear calls still not permitted.
